...
def normalize_db_url(url: str) -> str:
    # Render a veces entrega postgres://; SQLAlchemy + psycopg2 requiere postgresql+psycopg2://
...
def week_range_from_args(args):
    d_from, d_to, semana_sel = resolve_filters(args)
    if semana_sel:
        d_from, d_to = week_range(semana_sel)
    return d_from, d_to, semana_sel

@app.get("/registros")
def registros():
    d_from, d_to, semana_sel = week_range_from_args(request.args)
    db = SessionLocal()
    try:
        def between(q, col):
...
@app.post("/delete/<string:entity>/<int:item_id>")
def delete_item(entity, item_id):
    ...
    try:
        ...
        flash(f"Registro de {entity} eliminado.")
        return redirect(url_for("registros"))
    finally:
        db.close()
...
# ---------- Descargas CSV por entidad (para el menú desplegable) ----------
...
def download_entity(entity):
    ...
    try:
        ...
        if entity == "censo":
            ...
            writer = csv.DictWriter(buf, fieldnames=["fecha","censo_dia","censo_noche","total"])
            writer.writeheader()
            for r in rows:
                writer.writerow({
                    "fecha": r.fecha.isoformat(),
                    "censo_dia": r.censo_dia,
                    "censo_noche": r.censo_noche,
                    "total": r.total
                })
            ...
            writer = csv.DictWriter(buf, fieldnames=["fecha","horario","que_ocurrio","nombre_afectado","accion"])
            writer.writeheader()
            for r in rows:
                writer.writerow({
                    "fecha": r.fecha.isoformat(),
                    "horario": r.horario,
                    "que_ocurrio": r.que_ocurrio,
                    "nombre_afectado": r.nombre_afectado or "",
                    "accion": r.accion or ""
                })
            ...
            for r in rows:
                writer.writerow({
                    ...
                    "fecha": r.fecha.isoformat(),
                    "id": r.id_interno or "",
                    "empresa_contratista": r.empresa_contratista or "",
                    "descripcion_problema": r.descripcion_problema or "",
                    "tipo_riesgo": r.tipo_riesgo or "",
                    "pabellon": r.pabellon or "",
                    "habitacion": r.habitacion or "",
                    "ingresar_contacto": r.ingresar_contacto or "",
                    "nombre_usuario": r.nombre_usuario or "",
                    "responsable": r.responsable or "",
                    "estatus": r.estatus or "",
                    "notificacion_usuario": r.notificacion_usuario or "",
            ...
            for r in rows:
                writer.writerow({
                    ...
                    "q1_respuesta": r.q1_respuesta or "", "q1_puntaje": r.q1_puntaje if r.q1_puntaje is not None else "",
                    "q2_respuesta": r.q2_respuesta or "", "q2_puntaje": r.q2_puntaje if r.q2_puntaje is not None else "",
                    "q3_respuesta": r.q3_respuesta or "", "q3_puntaje": r.q3_puntaje if r.q3_puntaje is not None else "",
                    "q4_respuesta": r.q4_respuesta or "", "q4_puntaje": r.q4_puntaje if r.q4_puntaje is not None else "",
                    "q5_respuesta": r.q5_respuesta or "", "q5_puntaje": r.q5_puntaje if r.q5_puntaje is not None else "",
            ...
            writer = csv.DictWriter(buf, fieldnames=["fecha","tiempo_promedio_mmss","cantidad"])
            writer.writeheader()
            for r in rows:
                writer.writerow({
                    "fecha": r.fecha.isoformat(),
                    "tiempo_promedio_mmss": seconds_to_mmss(r.tiempo_promedio_sec),
                    "cantidad": r.cantidad
                })
...
def dashboard():
    ...
    try:
        ...
        # Encuestas (por fecha, cuenta registros)
        q = db.query(EncuestaEntry)
        if d_from: q = q.filter(EncuestaEntry.fecha_hora >= datetime.combine(d_from, time.min))
        if d_to:   q = q.filter(EncuestaEntry.fecha_hora <= datetime.combine(d_to, time.max))
        for r in q.all():
        ...
        s_censo, s_eventos, s_dup, s_enc, s_att_cant, s_att_min = [], [], [], [], [], []
        for k in ordered_days:
            ...
            s_att_min.append(round(prom_s/60.0, 2))
        ...
        cards = {
            ...
            "atencion_tiempo_prom_global": (
                seconds_to_mmss(int(mean([int(x*60) for x in s_att_min if x>0])))
                if any(x>0 for x in s_att_min) else "00:00"
            ),
        }
